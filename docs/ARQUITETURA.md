...
[docs/aprendizado/](aprendizado/)). Cada seção cita o arquivo e, quando útil,
a linha exata que sustenta a afirmação.
...
([src/routes/frontend.rs:705](../src/routes/frontend.rs)) só garante o token
...
- `holdings` tem chave primária composta `(user_id, asset_id)` — uma linha
  ...
  [src/repository.rs:581-598](../src/repository.rs)).
...
([src/repository.rs:272-316](../src/repository.rs)) são consultas triviais
...
[src/repository.rs:619-636](../src/repository.rs)). O comentário da migração
...
`MONEY_SCALE = 8` ([src/models.rs:12](../src/models.rs)) é o invariante mais
...
(`brl_price` em [src/quotes.rs:163-169](../src/quotes.rs)) grava `preço =
...
1. **Escrita passou a arredondar sempre.** `brl_price` arredonda para
   ...
   ([src/quotes.rs:168](../src/quotes.rs)); `validated_unit_value` faz o
   mesmo para qualquer escrita administrativa
   ([src/repository.rs:635](../src/repository.rs)); `buy_asset`/`sell_asset`
   arredondam o produto preço×quantidade antes de usá-lo
   (`round_dp(MONEY_SCALE)`, [src/repository.rs:481](../src/repository.rs)).
2. **Leitura passou a `ROUND(...)` defensivamente.** Todo agregado SQL que
   ...
   dentro do invariante" ([src/repository.rs:267-271](../src/repository.rs)).
...
([src/repository.rs:777-834](../src/repository.rs)) planta deliberadamente
...
- **Access token** (cookie `token`, [src/auth/user.rs:14](../src/auth/user.rs)) —
...
- **Refresh token** (cookie `refresh_token`,
  [src/auth/session.rs:22-36](../src/auth/session.rs)) — 32 bytes
...
([src/auth/session.rs:80-133](../src/auth/session.rs)), descrito na seção 2:
se o access expirou mas o refresh ainda é válido, `rotate_session`
([src/repository.rs:211-253](../src/repository.rs)) faz, **numa transação
...
Logout ([src/routes/frontend.rs:423-437](../src/routes/frontend.rs)) chama
...
header ([src/auth/admin.rs:39-41](../src/auth/admin.rs)) — "ele claramente
...
[src/routes/frontend.rs:269-298](../src/routes/frontend.rs)) — durante o
...
[src/app.rs:409-447](../src/app.rs)), aplicados a toda resposta: CSP
...
sistema — 17 variantes, cada uma mapeada para um `StatusCode` específico em
...
([src/error.rs:106-115](../src/error.rs)):
...
- `AppError::Jwt(String)` — **não** usa `#[from]`/`transparent` porque
  ...
  só a mensagem como string ([src/error.rs:60-64](../src/error.rs)).
...
[src/routes/frontend.rs:120-128](../src/routes/frontend.rs)); o handler
...
(`resolve` em [src/i18n.rs:63-68](../src/i18n.rs)). Formatação de dinheiro,
...
([src/app.rs:119-123](../src/app.rs)), nunca no caminho de uma requisição:
...
([src/app.rs:36-39](../src/app.rs)) é explícito — o snapshot de mercado
...
[src/app.rs:254-310](../src/app.rs)). Junto do trace, um histograma
...
[src/app.rs:326-334](../src/app.rs)), senão gerado localmente (8 bytes
...
**Migrações no boot** ([src/app.rs:54](../src/app.rs), dentro de
